...
object ALU {
  ...
  implicit class UIntToAugmentedUInt(private val x: UInt) extends AnyVal {
    ...
    def passBitsRight(hi: UInt, offset: Int = 0): UInt = {
      ...
      val hiOH      =
        if (offset >= 0) { UIntToOH(hi, maskWidth) << absOffset }
        else { UIntToOH(hi, maskWidth + absOffset) >> absOffset }
    ...
    def passBitsLeft(lo: UInt, offset: Int = 0): UInt = {
      ...
      val loOH      =
        if (offset >= 0) { UIntToOH(lo, maskWidth) << absOffset }
        else { UIntToOH(lo, maskWidth + absOffset) >> absOffset }
